...
import { z } from "zod";
import { Printer, Download, FileText, Loader2, ArrowLeft } from "lucide-react";
...
export const Route = createFileRoute("/contrato-electronico")({
  head: () => ({
    meta: [
      { title: "Contrato electrónico — Impulso Go" },
      { name: "description", content: "Genera un contrato electrónico A4 listo para imprimir o descargar como PDF." },
      { name: "robots", content: "noindex,nofollow" },
    ],
  }),
  component: ContratoElectronicoPage,
});
...
function ContratoElectronicoPage() {
    ...
    <div className="flex min-h-screen flex-col bg-background">
      ...
      <main className="flex-1 px-4 py-10 sm:px-6 sm:py-14">
        <div className="mx-auto max-w-5xl space-y-6 print-hide">
          ...
          <header>
            <p className="text-[11px] font-black uppercase tracking-[0.16em] text-action">Documento</p>
            <h1 className="text-3xl font-black tracking-tight text-institutional">Contrato electrónico</h1>
            <p className="text-sm text-muted-foreground">
              Genera un contrato A4 listo para imprimir o descargar como PDF. No reemplaza el flujo digital con folio firmado en línea.
            </p>
          </header>
          ...
          >
            ...
            <div className="sm:col-span-2 rounded-xl border border-border bg-surface-alt p-5">
              ...
              <div className="mt-5">
                ...
                <p className="mt-2 text-center text-3xl font-black tracking-tight text-institutional tabular-nums">{formatMXN(amount)}</p>
                <input
                  type="range"
                  min={10000}
                  max={250000}
                  ...
                  {TERMS.map((t) => (
                    <button
                      ...
                      className={`h-11 rounded-md text-sm font-black transition ${
                        termYears === t ? "bg-institutional text-white shadow-card-soft" : "bg-card text-foreground hover:bg-surface"
                      }`}
                    >
                      {t} años
                    </button>
            ...
            <div className="rounded-2xl border border-border bg-card p-6 shadow-card-soft">
              <div className="flex flex-wrap items-center justify-between gap-3">
                ...
                <div className="flex gap-2">
                  <button onClick={print} className="inline-flex items-center gap-2 rounded-md border border-border bg-card px-4 py-2 text-sm font-bold text-institutional hover:bg-accent">
